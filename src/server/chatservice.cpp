...
ChatService::ChatService()
{
    ...
    if (_redis.connect()) {
        // Set up Redis message handler, (int, string) channel number and message content
...
void ChatService::login(const TcpConnectionPtr& conn, json& js, Timestamp time)
{
    ...
    if (user.getPassword() == password && user.getId() == id) {
        ...
        if (user.getState() == "online") {
            ...
            // subscribe redis channel with the current user's id, to receive messages published to this channel
            _redis.subscribe(id);
...
void ChatService::clientCloseException(const TcpConnectionPtr& conn)
{
    ...
    if (user.getId() != -1) {
        // unsubscribe the user's Redis channel to stop receiving messages published to this channel
...
void ChatService::oneChat(const TcpConnectionPtr& conn, json& js, Timestamp time)
{
    ...
    // Check if user is online, he maybe on another server
    User user = _userModel.query(toId);
    if (user.getState() == "online") {
        // toId is online but not connected to this server, publish the message to Redis channel with toId as channel number
        _redis.publish(toId, js.dump());
        LOG_INFO << "User " << toId << " is online but not connected to this server, published message to Redis channel " << toId;
        return;
    }
...
void ChatService::groupChat(const TcpConnectionPtr& conn, json& js, Timestamp time)
{
    ...
    for (int userId : userIds) {
        ...
        if (it != _userConnMap.end()) {
            ...
            if (user.getState() == "online") {
                // User is online but not connected to this server, publish the message to Redis channel with
                // userId as channel number
...
void ChatService::logout(const TcpConnectionPtr& conn, json& js, Timestamp time)
{
    ...
    user.setState("offline");
    _userModel.updateState(user);
    LOG_INFO << "user id: " << user.getId() << " goes offline!";
}
...
void ChatService::handleRedisSubscribeMessage(int userId, string msg)
{
    ...
    {
        lock_guard<mutex> lock(_connMutex);
        // _userConnMap contains the users logged in to this server
        auto it = _userConnMap.find(userId);

        if (it != _userConnMap.end()) {
            // User is online, send the message directly
            it->second->send(msg);
            LOG_INFO << "Received message for user " << userId << " from Redis channel, forwarded to client.";
            return;
        }
    }
    ...
    LOG_INFO << "Received message for user " << userId << " from Redis channel, stored as offline message.";
}